...
class Gate(ABC):
    ...
    @abstractmethod
    def to_matrix(self, qubits_count: int) -> NDArray[np.complex128]:
        """Return the matrix representation for the given register size."""
...
class GateSingle(Gate):
    ...
    def __init__(self, gate2: Gate2, qubit_id: int) -> None:
        self.gate2 = gate2
    ...
    def to_matrix(self, qubits_count: int) -> NDArray[np.complex128]:
        ...
        if self.qubit_id == 0:
            ...
            for i in range(2):
                for j in range(2):
                    tile[i * ts2 : (i + 1) * ts2, j * ts2 : (j + 1) * ts2] = (
                        subtile * matrix[i, j]
                    )
...
class GateFC(Gate):
    ...
    def to_matrix(self, qubits_count: int) -> NDArray[np.complex128]:
        ...
        matrix = TwoLevelUnitary(
            self.gate2.to_matrix(), matrix_size, index1, index2
        )
...
def gates_to_matrix(
    gates: list[Gate], qubits_count: int
) -> NDArray[np.complex128]:
    """Converts gate sequence to matrix implemented by this sequence."""
    result = np.eye(2**qubits_count, dtype=np.complex128)
    for gate in gates:
...
def apply_on_qubit(gates: list[Gate2], qubit_id: int) -> list[GateSingle]:
